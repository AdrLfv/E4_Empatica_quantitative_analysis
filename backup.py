...
process_csv <- function(folder, file_name) {
    ...
    if (file.exists(file_path)) {
        ...
        if (file.exists(tags_path)) {
            ...
            # timestamp prend la valeur en tags_path[1,1] à convertir au format hh:mm;ss
            timestamp <- as.POSIXct(timestamp_unix, format = "%H:%M:%S", tz = "GMT") + (-4 * 3600)
            ...
            if (length(participant_index) > 0) {
              ...
              time_sequence <- seq(from = timestamp_video, by = 1/data[2, 1], length.out = nrow(data) - 2)
              time_sequence_formatted <- format(time_sequence, "%H:%M:%S")

              
              data$V5[:nrow(data)] <- time_sequence_formatted
...
# Fonction pour dessiner le graphe des pulsations en fonction du temps
draw_graph <- function(data, participant) {
    if (is.null(data)) {
    ...
    time <- as.POSIXct(data$V4[3:nrow(data)], format = "%H:%M:%S", tz = "GMT")
    pulse <- as.numeric(data$V1[3:nrow(data)])
    
    plot_data <- data.frame(Time = time, Pulse = pulse)
    
    p <- ggplot(plot_data, aes(x = Time, y = Pulse)) +
        geom_line() +
        labs(title = paste("Pulse over Time - Participant", participant),
             x = "Time (hh:mm:ss)",
             y = "Pulse (bpm)") +
        scale_x_datetime(date_labels = "%H:%M:%S", date_breaks = "1 min") +
        theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
        ylim(50, 205)
    
    # Sauvegarder le graphique
    participant_folder <- file.path(clear_data_path, participant)
    dir.create(participant_folder, showWarnings = FALSE, recursive = TRUE)
    
    ggsave(filename = file.path(participant_folder, "pulse_over_time_plot.png"),
           plot = p, width = 10, height = 5, units = "in")
    
    cat(paste("\nSaved pulse over time plot for participant", participant))
}


draw_graphs_passages <- function(data, file_name, participant) {
    if (is.null(data)) {
        warning("Les données sont nulles pour le participant", participant)
        return(NULL)
    }
...
for (folder in stream_folders) {
    participant <- substr(basename(folder), 1, 3)
    # si le participant n'est pas le 25 on skip
    # if (participant != "P25") {
    #     next
    # }
    cat(paste("\nProcessing", participant, "HR data"))

    hr_data <- process_csv(folder, "HR")
    
    participant_folder <- file.path(clear_data_path, participant)
    write.table(hr_data, file = file.path(participant_folder, "debug_HR.csv"), row.names = FALSE, sep = ";", col.names = FALSE)
    
    draw_graphs_passages(hr_data, "HR", participant)
    draw_graph(hr_data, participant)  # Ajouter cet appel pour générer le graphe des pulsations
    
    #cat(paste("\nProcessing", participant, "EDA data"))
    #eda_data <- process_csv(folder, "EDA")
    #draw_graphs_passages(eda_data, "EDA", participant)
}
